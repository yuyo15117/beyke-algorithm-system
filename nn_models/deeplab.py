...
import os
from nn_models.deeplabv3_plus import DeepLab
...
class DeeplabV3(object):
    _defaults = {
        ...
        #-------------------------------------------------------------------#
        "model_path"        : "E:\deeplabv3-plus-pytorch-main\model_data\\best_epoch_weights_xception_355_adm.pth",
        #----------------------------------------#
        #   所需要区分的类的个数+1
        #----------------------------------------#
        "num_classes"       : 2,
    ...
    def detect_image(self, image, count=False, name_classes=None):
        ...
        with torch.no_grad():
            ...
            pr = pr.argmax(axis=-1)
            # pr.show()
            save_dir = "E:\\deeplabv3-plus-pytorch-main\\result"
            # img_name = img_name.split('.')[0]
            # save_path = os.path.join(save_dir, f'{img_name}.png')
            # plt.imsave(save_path, pr, cmap='binary')
        #---------------------------------------------------------#
        #   计数
        #---------------------------------------------------------#
        if count:
